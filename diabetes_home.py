...
def app(diabetes_df):
    ...
    st.title("diabetes dataframe anaylisis")
 ...
 """)
    ...
    with beta_col2:
        if st.checkbox("View Columns Data Type"):
            st.table(list(diabetes_df.info()))
    # Add a checkbox in the third column followed by a selectbox which accepts the column name whose data needs to be displayed.
    with beta_col3:
        if st.checkbox("View Columns Data"):
            c_data = st.selectbox("select column",tuple(diabetes_df.columns))
            st.write(diabetes_df[c_data])